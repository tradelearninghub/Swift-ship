...
Customers get two primary search options on `/track`:

**Option 1 — AWB / Order ID:**
```
[__________]  [ TRACK ]
```
The system automatically detects whether the input is an AWB or a Booking/Order ID.
...
[ SEARCH ]
```
The system checks both `(Sender Mobile + Sender Pincode)` OR `(Receiver Mobile + Receiver Pincode)` — the customer doesn't need to specify which role they are.

---

## 26. Mobile + Pincode Duplicate Handling

- **One match** → direct tracking result shown.
- **Multiple matches** → a list is shown for the customer to pick from, e.g.:
  ```
  BK-1025   Jaipur → Mumbai   In Transit
  BK-1018   Jaipur → Delhi    Delivered
  BK-1009   Jaipur → Mumbai   Delivered
  ```

**[IMPROVED]** Rate-limit this search endpoint per IP/mobile number (see §53a) since it's a natural target for enumeration/scraping attacks — an attacker could brute-force mobile+pincode combinations to harvest shipment data.

---

## 27. Privacy in Tracking Results

**Show:** Booking ID, masked AWB, Origin City, Destination City, Current Status, Shipment Date.
**Never show:** full phone numbers, complete addresses, other sensitive customer data.
